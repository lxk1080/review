...
1. 连横智能埋点系统
   ...
   - 整体介绍以及运行流程？
     - 主要业务是：配置埋点+自动埋点
       - 对于一些需要重点关注的，配置埋点，可能需要准确的定位，使用 id 或 css、xpath
       - 对于普遍的流程，自动埋点，收集所有可被点击的元素，还有所有的图片，同时反馈此元素的 xpath 路径，文本，以及图片的 url
     ...
     - 区域的延迟上报功能，快速滚动，短时间内多元素曝光，此时不急着上报，等滚动结束，批量上报，主要用于收集用户的查看习惯
   - 具体的功能点有哪些（React + React-Native）？
     ...
     - React 中如何判断一个 dom 元素上是否绑有 click 事件？
       ...
       - 为什么需要找到那些可以被点击的元素？直接在 document 上做事件委托，只要一有点击就发不行吗？
         - 不太合理，如果用户在屏幕上一直点击，那岂不是一直发埋点吗
     ...
     - 其它的一些细节：
       ...
       - 诸如此类的细节因素还有很多。。
   - 那么 RN 是如何实现元素点击截取的？
     - 修改源码，我们没办法判断 RN 元素是否可以点击，但是可以在每一个可点击组件内插装一些代码：发送一个事件，参数包括当前元素的 xpath 路径和 tapId
     - 在 JSSDK 内我们监听这个事件即可，只要有元素被点击，JSSDK 就可以收到通知，主要 API：DeviceEventEmitter
   - 如何实现 Vue 版本？
