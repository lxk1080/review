...
3. 具体说说性能优化？
    - 接上一讲，首先需要排查问题，然后针对问题对症下药
    - 加载慢？（FP 之前的时间长）
      - 优化服务端配置，使用 CDN（静态文件服务，将源站内容分发至最接近用户的节点）和 DNS 预解析（对可能要打开的链接做 dns 预解析）
      - 对资源使用预加载（告诉浏览器立即加载资源）和预获取（告诉浏览器在空闲时才开始加载资源）
      - 使用 http2（请求头压缩、多路复用、服务端推送）
      - 减少主包的体积，路由懒加载或者拆分组件
      - 优化 http 缓存策略，强缓存和协商缓存
    - 渲染慢？（FP 到 L 的时间）
      - 从 js 代码分析
        - 对 for 循环中的条件做缓存（例如：`for (let i = 0; i < document.getElementsByTagName('p').length; i++) {}`，先把 length 赋值变量 len，不要重复获取）
        - 频繁的 Dom 操作，可以合并到一起操作（可以使用文档片段 API `createDocumentFragment`）
        - 节流（throttle）防抖（debounce）函数
        - requestAnimationFrame、requestIdleCallback 等 API 针对于 “帧” 的优化
      - 从 react 框架分析
        - 合理的利用 useCallback、useMemo 缓存数据
        - class 组件中使用 shouldComponentUpdate（默认返回 true），function 组件使用 React.memo 包裹子组件，并传递一个 props 比较函数（第二个参数）
        - 使用 useTransition、useDeferredValue（React18）标记一些非紧急的 state 更新，避免这类更新阻塞用户交互（如输入、点击等）
        - 使用 useOptimistic（React19）实现 “乐观更新” 模式，当操作成功时保留更新，失败时自动回滚到之前的状态，这个在点赞、评论等需要即时反馈的交互中体验很好
        - 使用不可变数据的一些库，例如 immutable、immer
        - 组件卸载时记得：副作用清除（定时器、各种响应事件）
      - 从打包工具分析（webpack）
        - 优化构建速度
          - 开启 babel 缓存
          - webpack.IgnorePlugin，用于忽略第三方包里面指定目录，让指定目录不被打包进去
          - happypack，开启多进程打包
          - HMR（热更新）
          - DllPlugin，动态链接库插件，用于开发环境（DllPlugin解决的是打包的速度），提高构建速度
        - 优化产出代码
          - 通用的优化：
            - 小图片 base64 编码，大图片压缩，代码压缩，断点续传（206）
            - bundle 加 hash，利用浏览器缓存
            - 动态导入（import），懒加载
          - Tree-shaking（生产模式自动开启）
          - @babel/plugin-transform-runtime
            - Babel 为编译的每个文件都插入了辅助代码，比如 _extend，使代码体积过大，可以将这些辅助代码作为一个独立模块，来避免重复引入
            - 还可以防止 babel-polyfill（core-js）注入造成全局污染
          - Code Split（使用 splitChunks 自定义如何分割代码，让主包变小）
          - 配置 optimization.runtimeChunk 优化缓存策略，解决 A 文件依赖 B 文件，当 B 文件改变（hash 改变），A 文件也跟着改变的问题，充分利用浏览器缓存
          - 对于体积很大的文件，使用 gzip 压缩，减小文件体积
      ...
      - 首屏优化（详见上文）
        ...
        - 骨架屏、loading 动画
      - 性能优化总结
        - 先找到问题，后解决问题，具体问题，具体对待


4. 后端一次性返回 10 万条数据，怎么渲染？
